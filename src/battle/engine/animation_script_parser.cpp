...
void __cdecl runAnimationScript(__int16 actor_id, void** ptrToScriptTable)
{
    ...
    __int16 v59; // ax
    __int16 v60; // ax
    FFContext* ffContext; // eax
    signed __int16 v62; // ax
    char v63; // al
    __int16 v64; // ax
    char v65; // al
    __int16 v66; // ax
    char v67; // [esp+4h] [ebp-6Ch]
    int v68; // [esp+8h] [ebp-68h]
    int v69; // [esp+38h] [ebp-38h]
    char v70; // [esp+40h] [ebp-30h]
    char v71; // [esp+41h] [ebp-2Fh]
    char v72; // [esp+42h] [ebp-2Eh]
    char v73; // [esp+43h] [ebp-2Dh]
    int v74; // [esp+44h] [ebp-2Ch]
    __int16 v75; // [esp+48h] [ebp-28h]
    char* v76; // [esp+4Ch] [ebp-24h]
    int v77; // [esp+50h] [ebp-20h]
    int v78; // [esp+54h] [ebp-1Ch]
    int v79; // [esp+58h] [ebp-18h]
    AnimScriptContext* scriptContextPtr; // [esp+5Ch] [ebp-14h]
    int vinAnimScriptIdx; // [esp+60h] [ebp-10h]
    ActivePartyMember* actorDataPtr; // [esp+64h] [ebp-Ch]
    _DWORD* newModelDataPtr; // [esp+68h] [ebp-8h]
    _DWORD* v84; // [esp+6Ch] [ebp-4h]

    if (!gamePausedGlobal)
    {
        GameAnimScriptContext* scriptContextPtr = ptrToScriptContext;
        byte_9ADEF8 = 0;
        ptrToScriptContext->isScriptActive = 1;     // executiong?
        ...
        LABEL_20:
            while (scriptContextPtr->isScriptActive)// while loop that runs the script
            {
                scriptContextPtr->currentOpCode = scriptContextPtr->scriptPtr[gBigAnimBlock[actor_id].currentScriptPosition++];
                switch (scriptContextPtr->currentOpCode)
                {
                case 0x8Eu:
                    if (!byte_BFB710)
                    {
