...
void initBattleCommandViewWidget(const BattleSpellDrawEvent* event) {
    ...
    auto battleCommandView = createWidget(BATTLE_SPELL_MENU_NAME);
    ...
    for (auto idx = 0; idx < names.size(); idx++) {
        ...
        gridParams = { cmdItemChoice, &battleCommandNameViewUpdater, 42, 360, allocateCommandRow };
    ...
    addChildWidget(mainWidget, battleCommandView, BATTLE_COMMAND_MENU_NAME);
}
